                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Ajouter une fiche de dépotage</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                aria-label="Close"></button>
                        </div>
                        <form action="{{ route('gerant.depotages') }}" method="post" enctype="multipart/form-data">
                            @csrf
    ...
    <div class="row justify-content-center">
        <div class="col-md-11 p-5 shadow bg-white">
            ...
            <table class="table text-center " style="padding-left: 10px;">
                <thead>
                    <tr>
                        <th class="text-start"  scope="col">Fiche</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-start">{{ $depotage->piece_jointe }}</td>
                        <td class="text-start">{{ $depotage->created_at }}</td>
                        <td ><a href="{{route("gerant.depotages") }}/{{ $depotage->id }}"><i class="fas fa-download"></i></a></td>
                        <td>
                            <form action="{{route("gerant.depotages") }}/{{ $depotage->id }}" method="post">
                                @csrf
                                @method("delete")
